  <!-- Holy Tome -->
  <div class="rpg-wpn-tome card wide-275 mar-20 text-white bg-yellow">
  <div class="accordion" id="accordion-tome">
      <div class="card-header text-center">
        <h4>Holy Tome</h4>
      </div>
      <div class="card-header pad-5 text-center">
        <button class="btn btn-link text-white text-decoration-none" type="button" data-toggle="collapse" data-target="#collapseFive" aria-expanded="true" aria-controls="collapseFive">
          <h6>Blessing</h6>
       </button>
      </div>
      <div id="collapseFive" class="collapse hidden" aria-labelledby="headingThree" data-parent="#accordion-tome">
        <div class="card-body light-transparency pad-10 text-center">
          <p class="card-text text-center">Call for a blessing on your ally. <br>Boosts an ally's DEF and RES.</p>
        </div>
        <ul class="list-group text-center">
          <li class="list-group-item align-items-center light-transparency bg-yellow" style="padding: .35rem 1.25rem;">
            <span class="badge badge-light text-green">+2</span>
            Bonus DEF
          </li>
        </ul>
        <ul class="list-group text-center">
          <li class="list-group-item align-items-center light-transparency bg-yellow" style="padding: .35rem 1.25rem;">
            <span class="badge badge-light text-blue">+2</span>
            Bonus RES
          </li>
        </ul>
      </div>
      <div class="card-header pad-5 text-center">
        <button class="btn btn-link text-white text-decoration-none" type="button" data-toggle="collapse" data-target="#collapseSix" aria-expanded="true" aria-controls="collapseSix">
          <h6>Healing Circle<span class="badge badge-light text-teal">5</span></h6>
        </button>
      </div>
      <div id="collapseSix" class="collapse hidden" aria-labelledby="headingFour" data-parent="#accordion-tome">
        <div class="card-body light-transparency pad-10 text-center">
          <p class="card-text text-center">Heal your entire party.<br>Critical Hits heal twice the amount. <br>HEAL = Weapon ATT + Player INT. <br>Costs 5 Mana.</p>
        </div>
          <ul class="list-group text-center">
          <li class="list-group-item align-items-center light-transparency bg-yellow" style="padding: .35rem 1.25rem;">
            <span class="badge badge-light text-orange">2</span>
            Weapon ATT
          </li>
        </ul>
      </div>
  </div><!-- /.card -->    
  </div><!-- /.accordion -->